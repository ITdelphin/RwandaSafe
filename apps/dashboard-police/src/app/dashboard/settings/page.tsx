...
export default function PoliceSettingsPage() {
    ...
    const [profileForm, setProfileForm] = useState({
        ...
        phone: user?.phone ?? '',
        badgeNumber: '',
        station: 'Kigali Central Police Station',
    });
    ...
    return (
        <div className="flex flex-col gap-6">
            <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-xl flex items-center justify-center text-white" style={{ background: `linear-gradient(135deg, ${ACCENT}, #154A6B)` }}>
                    <Settings size={20} />
                </div>
                <div>
                    ...
                    {TABS.map(({ id, label, icon: Icon }) => {
                        ...
                        return (
                            <button key={id} onClick={() => setActiveTab(id)} className="w-full flex items-center gap-2 px-3 py-2.5 rounded-xl text-[13px] mb-0.5 transition-all"
                                style={{ background: active ? '#EFF6FF' : 'transparent', color: active ? ACCENT : '#64748B', fontWeight: active ? 700 : 500 }}>
                                <Icon size={15} />{label}
                            </button>
                        );
                    })}
                ...
                <div className="flex-1 min-w-[300px]">
                    {activeTab === 'profile' && (
                        <div className="bg-white rounded-2xl border border-slate-200 p-7">
                            ...
                            <button onClick={saveProfile} className="mt-6 flex items-center gap-2 px-5 py-2.5 rounded-xl text-white text-[13px] font-bold" style={{ background: `linear-gradient(135deg, ${ACCENT}, #154A6B)` }}>
                    ...
                    {activeTab === 'notifications' && (
                        <div className="bg-white rounded-2xl border border-slate-200 p-7">
                            ...
                            <div className="flex flex-col gap-3">
                                {[
                                    { key: 'newIncident', label: 'New Incident Alert', desc: 'Get notified on new incidents' },
                                    { key: 'crimeAlert', label: 'Crime Alerts', desc: 'Immediate alerts for crime reports' },
                                    { key: 'dispatchAlert', label: 'Dispatch Alerts', desc: 'Receive dispatch and patrol assignments' },
                            ...
                            <button onClick={saveNotif} className="mt-6 flex items-center gap-2 px-5 py-2.5 rounded-xl text-white text-[13px] font-bold" style={{ background: `linear-gradient(135deg, ${ACCENT}, #154A6B)` }}>
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 p-7">
                            ...
                            <div className="flex flex-col gap-4 max-w-sm">
                                ...
                                <div className="flex gap-2 p-3 bg-blue-50 rounded-xl border border-blue-100">
                                    ...
                                    <p className="text-[11px] text-blue-700 m-0">Minimum 8 characters with letters and numbers.</p>
                                </div>
                            </div>
                            <button onClick={savePassword} className="mt-5 flex items-center gap-2 px-5 py-2.5 rounded-xl text-white text-[13px] font-bold" style={{ background: `linear-gradient(135deg, ${ACCENT}, #154A6B)` }}>
