...
export default function HospitalSettingsPage() {
    ...
    const [profileForm, setProfileForm] = useState({
        ...
        phone: user?.phone ?? '',
        staffId: '',
        facility: 'CHUK – Centre Hospitalier Universitaire de Kigali',
    });
    ...
    return (
        <div className="flex flex-col gap-6">
            <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-xl flex items-center justify-center text-white" style={{ background: `linear-gradient(135deg, ${ACCENT}, #A51B1B)` }}>
                    <Settings size={20} />
                </div>
                <div>
            ...
            <div className="flex gap-5 flex-wrap">
                <div className="bg-white rounded-2xl border border-slate-200 p-2 h-fit min-w-[180px]">
                    {TABS.map(({ id, label, icon: Icon }) => {
                        ...
                        return (
                            <button key={id} onClick={() => setActiveTab(id)} className="w-full flex items-center gap-2 px-3 py-2.5 rounded-xl text-[13px] mb-0.5 transition-all"
                                style={{ background: active ? '#FEF2F2' : 'transparent', color: active ? ACCENT : '#64748B', fontWeight: active ? 700 : 500 }}>
                                <Icon size={15} />{label}
                            </button>
                        );
                    })}
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 p-7">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                {[
                                    ...
                                    { label: 'Medical Facility', key: 'facility' },
                                ].map((f) => (
                                    <div key={f.key} style={{ gridColumn: f.key === 'facility' ? 'span 2' : undefined }}>
                                        ...
                                        <input value={(profileForm as any)[f.key]} onChange={(e) => setProfileForm({ ...profileForm, [f.key]: e.target.value })}
                                            className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-[13px] bg-slate-50 text-slate-900 outline-none" />
                                    </div>
                                ))}
                            </div>
                            <button onClick={saveProfile} className="mt-6 flex items-center gap-2 px-5 py-2.5 rounded-xl text-white text-[13px] font-bold" style={{ background: `linear-gradient(135deg, ${ACCENT}, #A51B1B)` }}>
                    ...
                    {activeTab === 'notifications' && (
                        <div className="bg-white rounded-2xl border border-slate-200 p-7">
                            ...
                            <div className="flex flex-col gap-3">
                                {[
                                    { key: 'newCase', label: 'New Emergency Case', desc: 'Alert on new incoming emergency patients' },
                                    { key: 'dispatchAlert', label: 'Ambulance Dispatch', desc: 'Receive ambulance dispatch alerts' },
                                    { key: 'capacityAlert', label: 'Capacity Warnings', desc: 'Alert when facility capacity is critical' },
                                    { key: 'telemedicine', label: 'Telemedicine Requests', desc: 'Receive incoming telemedicine session alerts' },
                                    ...
                                    <div key={item.key} className="flex justify-between items-center p-3.5 bg-slate-50 rounded-xl">
                                        ...
                                        <button onClick={() => setNotifSettings({ ...notifSettings, [item.key]: !(notifSettings as any)[item.key] })}
                            ...
                            <button onClick={saveNotif} className="mt-6 flex items-center gap-2 px-5 py-2.5 rounded-xl text-white text-[13px] font-bold" style={{ background: `linear-gradient(135deg, ${ACCENT}, #A51B1B)` }}>
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 p-7">
                            ...
                            <div className="flex flex-col gap-4 max-w-sm">
                                {[
                                    ...
                                    <div key={field.key}>
                                        ...
                                        <div style={{ position: 'relative' }}>
                                            <input type={field.show ? 'text' : 'password'} value={(secForm as any)[field.key]}
                                                onChange={(e) => setSecForm({ ...secForm, [field.key]: e.target.value })}
                                                className="w-full px-3.5 py-2.5 pr-10 rounded-xl border border-slate-200 text-[13px] bg-slate-50 text-slate-900 outline-none" />
                                ...
                                <div className="flex gap-2 p-3 bg-red-50 rounded-xl border border-red-100">
                                    <AlertCircle size={14} color={ACCENT} style={{ flexShrink: 0, marginTop: '1px' }} />
                                    <p className="text-[11px] text-red-700 m-0">Minimum 8 characters with letters and numbers.</p>
                                </div>
                            </div>
                            <button onClick={savePassword} className="mt-5 flex items-center gap-2 px-5 py-2.5 rounded-xl text-white text-[13px] font-bold" style={{ background: `linear-gradient(135deg, ${ACCENT}, #A51B1B)` }}>
