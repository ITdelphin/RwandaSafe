...
const AGENCY = { icon: <Stethoscope size={20} />, label: 'SAMU Medical', accent: '#C62828' };

const NAV = [
  { href: '/dashboard', label: 'Overview', icon: <LayoutDashboard size={18} /> },
  { href: '/dashboard/incidents', label: 'Emergency Cases', icon: <Siren size={18} /> },
  { href: '/dashboard/dispatch', label: 'Dispatch', icon: <Radio size={18} /> },
  { href: '/dashboard/capacity', label: 'Hospital Capacity', icon: <Building2 size={18} /> },
        ...
        <div className="px-6 py-6" style={{ borderBottom: '1px solid #1E293B' }}>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-xl flex-shrink-0"
              style={{ background: 'linear-gradient(135deg, #C62828, #A51B1B)' }}>
              {AGENCY.icon}
            </div>
          ...
          {NAV.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href}
                ...
                <span className={`transition-colors duration-200 ${active ? 'text-red-400' : 'text-slate-500 group-hover:text-slate-300'}`}>
                  {item.icon}
                </span>
          ...
          <div className="p-3 rounded-2xl bg-[#1E293B]/40 border border-[#1E293B]/50 backdrop-blur-sm shadow-inner transition-all duration-200 hover:bg-[#1E293B]/60">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-xs font-bold truncate text-white tracking-tight">{user?.name ?? 'Medical'}</div>
                <div className="text-[10px] truncate text-slate-500 font-medium">{user?.role?.replace(/_/g, ' ') ?? 'Medical Responder'}</div>
              </div>
            </div>
          </div>
