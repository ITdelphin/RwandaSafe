    ...
    <div className="relative w-full h-full">
      ...
      <div className="absolute top-4 right-4 bg-white rounded-xl shadow-lg p-4 w-52">
        ...
        <button onClick={() => setHeatMode(h => !h)}
          className={`w-full text-xs py-1.5 rounded-lg font-medium transition-colors flex items-center justify-center gap-1.5 ${heatMode ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
          <Thermometer size={14} />
      ...
      {!process.env.NEXT_PUBLIC_GOOGLE_MAPS_KEY || process.env.NEXT_PUBLIC_GOOGLE_MAPS_KEY === 'your_google_maps_key' ? (
        <div className="absolute inset-0 flex items-center justify-center bg-gray-100">
          <div className="text-center">
            <div className="flex justify-center mb-3 text-gray-400"><Map size={48} /></div>
