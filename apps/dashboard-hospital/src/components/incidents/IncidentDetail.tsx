...
import { AmbulanceDispatchPanel } from '../ambulance/AmbulanceDispatchPanel';
import { HospitalRecommendation } from '../capacity/HospitalRecommendation';
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
          {!isLoading && incident && (
            <>
              ...
              {/* Medical sections */}
              <PatientCard incidentId={incidentId} medicalCase={incident.medicalCase} onRefresh={refresh} />
              <AmbulanceDispatchPanel incidentId={incidentId} medicalCase={incident.medicalCase} onRefresh={refresh} />
              {(['MEDICAL_EMERGENCY','ACCIDENT'].includes(incident.type)) && (
                <HospitalRecommendation incidentId={incidentId} onAssigned={refresh} />
              )}
              {incident.type === 'MEDICAL_EMERGENCY' && (
                <TelemedicinePanel incidentId={incidentId} citizenUserId={incident.reporterId} existingSession={incident.telemedicineSessions?.[0]} />
              )}
