...
const AGENCY = { icon: <Flame size={20} />, label: 'Fire Brigade', accent: '#EA580C' };
    ...
    <>
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <div className="text-[17px] font-bold text-white tracking-tight leading-tight">
                Safe<span className="text-orange-500">Rwanda</span>
              </div>
          ...
          {NAV.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href}
                ...
                <span className={`transition-colors duration-200 ${active ? 'text-orange-500' : 'text-slate-500 group-hover:text-slate-300'}`}>
                  {item.icon}
                </span>
        ...
        <div className="p-4" style={{ borderTop: '1px solid #1E293B' }}>
          <div className="p-3 rounded-2xl bg-[#1E293B]/40 border border-[#1E293B]/50 backdrop-blur-sm shadow-inner group transition-all duration-200 hover:bg-[#1E293B]/60">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-xs font-bold truncate text-white tracking-tight">{user?.name ?? 'Responder'}</div>
                <div className="text-[10px] truncate text-slate-500 font-medium">{user?.role?.replace(/_/g, ' ') ?? 'Fire Officer'}</div>
              </div>
            </div>
          </div>
          ...
          <button
            onClick={logout}
            className="mt-3 w-full flex items-center justify-center gap-2 text-[11px] font-bold px-3 py-2.5 rounded-xl transition-all duration-200 hover:bg-red-500/10 text-red-500/80 hover:text-red-500 group border border-transparent hover:border-red-500/20"
          >
