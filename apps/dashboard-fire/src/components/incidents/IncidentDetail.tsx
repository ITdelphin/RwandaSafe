...
import { FireReportPanel } from '../fire/FireReportPanel';
...
export function IncidentDetail({ incidentId, onClose }: Props) {
  ...
  const { data: incident, isLoading } = useQuery({
    ...
    queryFn: () => incidentsApi.getById(incidentId).then(r => (r.data as any).data),
    enabled: !!incidentId,
  });
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
          {!isLoading && incident && (
            <>
              ...
              {/* Fire Report Panel */}
              {['FIRE', 'NATURAL_DISASTER'].includes(incident?.type) && (
                <FireReportPanel incidentId={incidentId} fireReport={incident?.fireReport} onRefresh={refresh} />
              )}
