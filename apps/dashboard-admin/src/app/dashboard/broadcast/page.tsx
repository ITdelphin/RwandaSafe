        ...
        <form onSubmit={handleSend} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: '16px' }}>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: '6px', flexWrap: isMobile ? 'wrap' : undefined }}>
                {SEVERITIES.map((s) => (
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
            <button type="submit" disabled={sendMutation.isPending}
              ...
              <Send size={16} /> {sendMutation.isPending ? 'Sending...' : 'Send Alert'}
            </button>
          </div>
        </form>
      </div>
      ...
      <div style={{ background: '#fff', borderRadius: '14px', border: '1px solid #E2E8F0', overflow: 'auto' }}>
        ...
        {isMobile ? (
          <div style={{ padding: '12px' }}>
            {alerts?.length === 0 ? (
              <p style={{ textAlign: 'center', padding: '40px', color: '#94A3B8', fontSize: '13px' }}>No broadcast alerts sent yet</p>
            ) : (
              alerts?.map((a: any) => (
                <div key={a.id} style={{ background: '#F8FAFC', borderRadius: '10px', padding: '14px', marginBottom: '8px', border: '1px solid #E2E8F0' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                    <div style={{ fontWeight: 600, color: '#0F172A', fontSize: '14px' }}>{a.title}</div>
                    <span style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: '#94A3B8' }}>
                    <span>{a.district ?? 'All Rwanda'} — {a.deliveredCount ?? 0}/{a.targetCount ?? 0} delivered</span>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      ...
                      {a.isActive && (
                        <button onClick={() => deactivateMutation.mutate(a.id)}
                          style={{ padding: '4px 8px', borderRadius: '4px', border: '1px solid #FECACA', background: '#FEF2F2', color: '#DC2626', fontSize: '10px', fontWeight: 600, cursor: 'pointer' }}>
                          Deactivate
                        </button>
                      )}
                    </div>
                    ...
                    <td style={tdStyle}>
                      {a.isActive && (
                        <button onClick={() => deactivateMutation.mutate(a.id)}
                          ...
                          <XCircle size={12} style={{ marginRight: '4px', verticalAlign: 'middle' }} /> Deactivate
                        </button>
                      )}
                    </td>
                  </tr>
