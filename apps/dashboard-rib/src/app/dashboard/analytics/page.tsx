...
export default function AnalyticsPage() {
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: () => dashboardApi.getStats('RIB').then((r: any) => r.data.data),
    refetchInterval: 60000,
  });

  const { data: incidents = [] } = useQuery({
    queryKey: ['analytics-incidents', range, typeFilter],
    queryFn: () => dashboardApi.getIncidents({ agencyType: 'RIB', limit: 200, type: typeFilter || undefined }).then((r: any) => {
      const d = r.data.data;
