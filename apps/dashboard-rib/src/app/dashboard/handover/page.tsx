...
export default function HandoverPage() {
  ...
  const { data: openCases = [] } = useQuery({
    ...
    queryFn: () => dashboardApi.getIncidents({ agencyType: 'RIB', status: 'ASSIGNED', limit: 50 }).then((r: any) => {
      const d = r.data.data;
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        {openCases.length === 0 ? (
          <EmptyState message="No open cases assigned to you" icon={<CheckCircle2 size={40} />} />
        ) : (
