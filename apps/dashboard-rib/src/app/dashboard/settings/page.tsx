...
export default function RibSettingsPage() {
    ...
    const [profileForm, setProfileForm] = useState({
        ...
        phone: user?.phone ?? '',
        badgeNumber: '',
        unit: 'Kigali Investigation Unit',
    });
    ...
    return (
        <div className="flex flex-col gap-6">
            <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-xl flex items-center justify-center text-white" style={{ background: `linear-gradient(135deg, ${ACCENT}, #371A6D)` }}>
                    <Settings size={20} />
                </div>
                <div>
                    ...
                    {TABS.map(({ id, label, icon: Icon }) => {
                        ...
                        return (
                            <button key={id} onClick={() => setActiveTab(id)} className="w-full flex items-center gap-2 px-3 py-2.5 rounded-xl text-[13px] mb-0.5 transition-all"
                                style={{ background: active ? '#F5F3FF' : 'transparent', color: active ? ACCENT : '#64748B', fontWeight: active ? 700 : 500 }}>
                                <Icon size={15} />{label}
                            </button>
                        );
                    })}
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 p-7">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                {[
                                    ...
                                    { label: 'Investigation Unit', key: 'unit' },
                                ].map((f) => (
                                    <div key={f.key} style={{ gridColumn: f.key === 'unit' ? 'span 2' : undefined }}>
                            ...
                            <button onClick={saveProfile} className="mt-6 flex items-center gap-2 px-5 py-2.5 rounded-xl text-white text-[13px] font-bold" style={{ background: `linear-gradient(135deg, ${ACCENT}, #371A6D)` }}>
                    ...
                    {activeTab === 'notifications' && (
                        <div className="bg-white rounded-2xl border border-slate-200 p-7">
                            ...
                            <div className="flex flex-col gap-3">
                                {[
                                    { key: 'newCase', label: 'New Investigation Case', desc: 'Alert when a new case is opened' },
                                    { key: 'tipSubmission', label: 'Anonymous Tips', desc: 'Receive new tip submissions' },
                                    { key: 'patternAlert', label: 'Crime Pattern Alerts', desc: 'Alert when a new crime pattern is detected' },
                                    { key: 'dailyReport', label: 'Daily Summary', desc: 'Receive daily case summary report' },
                            ...
                            <button onClick={saveNotif} className="mt-6 flex items-center gap-2 px-5 py-2.5 rounded-xl text-white text-[13px] font-bold" style={{ background: `linear-gradient(135deg, ${ACCENT}, #371A6D)` }}>
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 p-7">
                            ...
                            <div className="flex flex-col gap-4 max-w-sm">
                                ...
                                <div className="flex gap-2 p-3 bg-purple-50 rounded-xl border border-purple-100">
                                    ...
                                    <p className="text-[11px] text-purple-700 m-0">Minimum 8 characters with letters and numbers.</p>
                                </div>
                            </div>
                            <button onClick={savePassword} className="mt-5 flex items-center gap-2 px-5 py-2.5 rounded-xl text-white text-[13px] font-bold" style={{ background: `linear-gradient(135deg, ${ACCENT}, #371A6D)` }}>
